...
class TerminalPlusView extends View
  ...
  @content: ->
    @div class: 'terminal-plus terminal-view', outlet: 'terminalPlusView', =>
      ...
      @div class: 'xterm', outlet: 'xterm', =>
        @div class: 'scroll_container', outlet: 'container', =>
          @div class: 'scroll_slider', outlet: 'slider'
  ...
  displayTerminal: ->
    ...
    @container.on 'scroll', (ev) =>
      @scrollXterm()

    @container.bind 'mousewheel DOMMouseScroll', (ev) =>
      #if ev.originalEvent.wheelDelta > 0 or ev.originalEvent.detail < 0
  ...
  scrollXterm: ->
    @terminal.ydisp = Math.round( @container.scrollTop() / (@slider.height() - @container.height()) * @terminal.ybase )
    if @terminal.ydisp < 0
      @terminal.ydisp = 0
    else if @terminal.ydisp > @terminal.ybase
      @terminal.ydisp = @terminal.ybase
  ...
  attachListeners: ->
    @ptyProcess.on 'terminal-plus:data', (data) =>
      @terminal.write data
      if @slider
        @slider.height @terminal.ybase * @rowHeight
        @container.scrollTop @slider.height() - @container.height()
